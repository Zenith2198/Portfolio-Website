...
import { getBaseUrl, fixDate, smartTrim, fixPath } from "@/lib/utils";
...
export default function HoverCard({ posts, contentLen = 500 }: { posts: Array<PostWithChapters>, contentLen?: number }) {
	return (
		<div className="card w-96 bg-neutral text-primary-content max-w-full items-stretch overflow-hidden">
			{posts.map((post, i) => (
				<Link href={`${getBaseUrl()}/${post.postTypeId}/${fixPath(post.path)}`} key={i} className="card-body p-0 relative border-base-300 [&:not(:first-child)]:border-t [&:not(:last-child)]:border-b h-full group">
