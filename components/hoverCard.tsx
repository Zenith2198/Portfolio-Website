...
export default function HoverCard({ posts, contentLen = 500 }: { posts: Array<Post>, contentLen?: number }) {
	return (
		<div className="card w-96 bg-neutral text-primary-content max-w-full items-stretch overflow-hidden">
			{posts.map((post, i) => (
				<Link href={`/blogs/${post.path}`} key={i} className="card-body p-0 relative border-base-300 [&:not(:first-child)]:border-t [&:not(:last-child)]:border-b h-full group">
					...
					<div className={`card px-10 py-5 absolute bottom-0 left-0 right-0 top-0 h-full w-full overflow-hidden bg-black bg-fixed opacity-0 transition duration-300 ease-in-out group-hover:opacity-[90%] ${i===0?"rounded-b-none":""} ${i===posts.length-1?"rounded-t-none":""}`}>
